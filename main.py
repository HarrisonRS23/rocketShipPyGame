...
BORDER = pygame.Rect(
    ...
    10,
    HEIGHT,
)  # -5 to have rect in middle of screen
...
YELLOW_HIT = pygame.USEREVENT + 1  # different event id based on number added
...
def handel_bullets(yellow_bullets, red_bullets, yellow, red):
    for bullet in yellow_bullets:
        bullet.x += BULLET_VEL
        if red.colliderect(bullet):  # check if rect has collided
            ...
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_q and len(yellow_bullets) < MAX_BULLETS:
                    bullet = pygame.Rect(
                        ...
                        10,
                        5,
                    )  # width of bullet 10 and height 5
